---
title: "On the adequacy of fruit removal as a proxy for fitness in studies of phenotypic selection"
...
```{r}
data <- read.table("C:/RD/Passiflora_fruits.txt", head = TRUE)
...
summarise(xseedn = mean(seedn_obs, na.rm = TRUE),
          xdiam = mean(diameter_mm, na.rm = TRUE),
          xmass = mean(mass_g, na.rm = TRUE),
          cvdiam = 100*sd(diameter_mm, na.rm = TRUE)/mean(diameter_mm, na.rm = TRUE),
          cvmass = 100*sd(mass_g, na.rm = TRUE)/mean(mass_g, na.rm = TRUE),
          cvseedn = 100*sd(seedn_obs, na.rm = TRUE)/mean(seedn_obs, na.rm = TRUE),
          sddiam = sd(diameter_mm, na.rm = TRUE),
          sdmass = sd(mass_g, na.rm = TRUE),
          sdseedn = sd(seedn_obs, na.rm = TRUE),
          n = n())
```
...
rel.importance <- calc.relimp(total.fitness.model, b = 1000, type = "lmg")
rel.importance
```
...
newdata <- data.frame(data[, c("wrel3", "z1", "z2", "z3")])
...
results <- data.frame(grad = selection.gradients,
